...
for i in W35PASS/*.vcf.gz ; do bcftools view -f PASS $i -Oz -o $i.out; done

for i in W36PASS/*.vcf.gz ; do bcftools view -f PASS $i -Oz -o $i.out; done
...
jdk-16.0.2/bin/java -Djava.io.tmpdir=tmp -jar picard.jar LiftoverVcf I=W_dedup.vcf.gz O=W_merged_hg19.vcf.gz CHAIN=hg38ToHg19.over.chain REJECT=rejected_variants.vcf R=ucsc.hg19.fasta TMP_DIR=tmp MAX_RECORDS_IN_RAM=10000

# Rename chromosomes:
bcftools annotate --rename-chrs chr_names.txt W_merged_hg19.vcf.gz  -Oz -o W_hg19_chr.vcf.gz

tabix -p vcf W_hg19_chr.vcf.gz

# Include SNPs for dbSNP:
bcftools annotate -c ID -a All_20161121.vcf.gz W_hg19_chr.vcf.gz > W_merged_FINAL.vcf.gz
